...
public interface PlanetRepository extends JpaRepository<Planet, Long> {
    ...
    @Query(value = "SELECT new com.mi.planetmanagement.dto.PlanetsSortedBySatDTO(count(s.id) as satellitesNumber, p) " +
                   "FROM Planet p LEFT JOIN Satellite s ON p.id = s.planet.id GROUP BY p")
